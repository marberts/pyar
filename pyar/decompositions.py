...
def hallerbach(p1, p0, v1, v0):
    p1, p0, v1, v0 = _balance_nas(p1, p0, v1, v0)
    r = p1 / p0
    v0, v1 = scale_weights(v0), scale_weights(v1 / r)
    laspeyres, paasche = mean(r, v0, skipna=True), mean(r, v1, skipna=True)
    return (
...
def diewert(p1, p0, v1, v0):
    ...
    r = p1 / p0
    laspeyres, paasche = (
        mean(r, v0, skipna=True),
        mean(r, v1, order=-1.0, skipna=True),
    )
    fisher = mean((laspeyres, paasche), order=0.0, skipna=True)
...
def vonBortkiewicz(p1, p0, v1, v0, *, type: str = "arithmetic"):
    ...
    if type == "arithmetic":
        q1, q0 = v1 / p1, v0 / p0
        price, quantity = (
            mean(p1 / p0, v0, skipna=True),
            mean(q1 / q0, v1, skipna=True),
        )
        return scale_weights(v0) * (p1 / p0 / price - 1) * (q1 / q0 / quantity - 1)
    elif type == "geometric":
        v1, v0 = scale_weights(v1), scale_weights(v0)
        r = p1 / p0
        return v0 * (v1 / v0 - 1) * np.log(r / mean(r, v0, order=0, skipna=True))


def contributions(x: pd.Series, weights=pd.Series, order: float = 0.0) -> pd.Series:
